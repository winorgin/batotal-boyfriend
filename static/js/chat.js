...
async function loadUserInfo() {
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('intimacy').textContent = stats.intimacy || 0;
            document.getElementById('dol').textContent = stats.dolBalance || 0;
...
async function loadChatHistory() {
    try {
        ...
        const data = await response.json();
        const messages = data.data?.messages || data.history || [];
        if (data.success && messages.length > 0) {
            const messagesDiv = document.getElementById('messages');
            messagesDiv.innerHTML = '';
            
            // DB 行格式：{message: 用户消息, reply: AI回复}
            // 按时间升序展示（API 返回降序，需翻转）
            [...messages].reverse().forEach(msg => {
                if (msg.role && msg.content) {
                    // 已格式化的 {role, content} 格式
                    addMessageToUI(msg.role, msg.content, false);
                } else {
                    // 数据库原始行格式 {message, reply}
                    if (msg.message) addMessageToUI('user', msg.message, false);
                    if (msg.reply)   addMessageToUI('assistant', msg.reply, false);
                }
            });
...
async function sendMessage() {
    ...
    try {
        ...
        if (data.success) {
            // AI 回复直接从 HTTP 响应中获取（WebSocket 仅用于附加推送）
            const payload = data.data || {};
            const reply   = payload.reply || payload.response;
            const audioUrl = payload.voiceUrl || payload.audioUrl;
            const intimacyChange = payload.intimacyChange;
            const newIntimacy    = payload.newIntimacy;

            hideTypingIndicator();

            if (reply) {
                if (audioUrl) {
                    addMessageWithAudioToUI('assistant', reply, audioUrl);
                } else {
            ...
            if (intimacyChange) {
                document.getElementById('intimacy').textContent = newIntimacy;
